...
Implements the full GhostGate spec:
  ...
  • Structural Audit      — chunk contributions, ghost signals, order weights
"""
...
class GlassboxNetV2(nn.Module):
    ...
    chunk_sizes:      list of input feature counts per chunk (e.g. [3, 3, 2])
    ...
    def forward(self, x, return_audit=False):
        ...
        if self.use_order_decomp:
            for i in range(self.n_chunks):
                cname = f'Chunk{i}'
                beta        = all_norms.get(f'{cname}_order_beta')
                lin_norm    = all_norms.get(f'{cname}_linear_norm')
                nonlin_norm = all_norms.get(f'{cname}_nonlinear_norm')
                if beta is not None:
    ...
    def get_gate_l1_loss(self) -> torch.Tensor:
        """L1 regularisation on ghost gates AND order gates."""
        losses = []

        if self.use_ghost:
            # Per-sample logits stored by each gate during last forward pass
        ...
        if self.use_order_decomp:
            order_betas = torch.stack([
                chunk.order_gate for chunk in self.chunks
            ])
